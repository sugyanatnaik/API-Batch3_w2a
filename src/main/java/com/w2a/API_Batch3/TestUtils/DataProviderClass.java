...
public class DataProviderClass extends APISetUp {
	 ...
	 * sheetName = m.getName(); int rows = excel.getRowCount(sheetName);// 3 int
	 * cols = excel.getColumnCount(sheetName);// 2 // int[][] array= new int[5][4];
	 * Object[][] data = new Object[rows - 1][1];// [2][1] // int[] array= new
	 * int[1][1]; // array[0][0]=10 Hashtable<String, String> table = null; for (int
	 * rowNum = 2; rowNum <= rows; rowNum++) {// rows=3
	 * 
	 * table = new Hashtable<String, String>(); for (int colNum = 0; colNum < cols;
	 * colNum++) {// cols=2 table.put(excel.getCellData(sheetName, colNum, 1),
